...
object NewPipeStreamExtractor {
    ...
    private val inFlightStreamInfo = ConcurrentHashMap<String, Deferred<StreamInfo?>>()
     ...
     */
    private suspend fun fetchStreamInfo(videoId: String): StreamInfo? = coroutineScope {
        val cachedInfo = synchronized(cacheLock) { streamInfoCache.get(videoId) }
        if (cachedInfo != null) return@coroutineScope cachedInfo

        val deferred = inFlightStreamInfo.computeIfAbsent(videoId) {
            extractorScope.async {
                try {
                    ...
                    inFlightStreamInfo.remove(videoId)
                }
            }
        }
    ...
    /**
     * Extract the best muxed video+audio stream URL for Music Video playback.
     * Prefers 720p, then falls back to 480p, 360p, or whatever is available.
     * Returns null if no video streams are found (audio-only content).
     */
    suspend fun getVideoStreamUrl(videoId: String?): String? = coroutineScope {
        if (videoId.isNullOrEmpty()) return@coroutineScope null

        val info = fetchStreamInfo(videoId) ?: return@coroutineScope null
        val uploader = info.uploaderName ?: ""
        val title = info.name ?: ""
        val desc = info.description?.toString() ?: ""
        val isArtTrack = uploader.endsWith("- Topic", ignoreCase = true) ||
                desc.contains("Provided to YouTube by", ignoreCase = true) ||
                desc.contains("Auto-generated by YouTube", ignoreCase = true) ||
                title.contains("Official Audio", ignoreCase = true) ||
                title.contains("(Audio)", ignoreCase = true) ||
                title.contains("[Audio]", ignoreCase = true)
        if (isArtTrack) {
            Log.d(TAG, "Video $videoId is an Art Track or Audio-only (no real MV). Suppressing MV stream.")
            return@coroutineScope null
        }
        val videoStreams = info.videoStreams
        if (videoStreams.isNullOrEmpty()) {
            Log.d(TAG, "No muxed video streams found for $videoId")
            return@coroutineScope null
        }
        ...
        val url = target?.content
        if (url != null) {
            Log.d(TAG, "Selected video stream for $videoId: ${target.resolution} (${target.format?.name})")
        } else {
            Log.d(TAG, "No usable video stream URL for $videoId")
        }
