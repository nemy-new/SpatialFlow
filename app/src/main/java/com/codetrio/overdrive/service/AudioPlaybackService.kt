...
package com.codetrio.overdrive.service
...
class AudioPlaybackService : MediaSessionService() {
    ...
    companion object {
        ...
        const val ACTION_PLAY = "com.codetrio.overdrive.ACTION_PLAY"
        const val ACTION_PAUSE = "com.codetrio.overdrive.ACTION_PAUSE"
        const val ACTION_PREVIOUS = "com.codetrio.overdrive.ACTION_PREVIOUS"
        const val ACTION_NEXT = "com.codetrio.overdrive.ACTION_NEXT"
        const val ACTION_TOGGLE_LOOP = "com.codetrio.overdrive.ACTION_TOGGLE_LOOP"
        const val ACTION_CYCLE_PLAYBACK_MODE = "com.codetrio.overdrive.ACTION_CYCLE_PLAYBACK_MODE"
        const val ACTION_TOGGLE_FAV = "com.codetrio.overdrive.ACTION_TOGGLE_FAV"
    ...
    private lateinit var telemetryManager: com.codetrio.overdrive.data.innertube.YouTubeTelemetryManager
    ...
    override fun onCreate() {
        ...
        telemetryManager = com.codetrio.overdrive.data.innertube.YouTubeTelemetryManager(
            context = this,
            httpClient = com.codetrio.overdrive.data.innertube.InnerTubeClient.httpClient,
            clientProvider = { com.codetrio.overdrive.data.innertube.InnerTubeClient }
        )
    ...
    private fun resolveAndLoad(uri: Uri) {
        ...
        if (uri.scheme == "innertube") {
            ...
            resolveJob = serviceScope.launch {
                ...
                val streamUrl = (streamResult as? com.codetrio.overdrive.domain.error.Result.Success)?.data
                    ...
                    if (pendingSeekPos <= 0) {
                        launch(Dispatchers.IO) {
                            val position = kotlinx.coroutines.withTimeoutOrNull(2000L.milliseconds) {
                                ...
                                (posResult as? com.codetrio.overdrive.domain.error.Result.Success)?.data ?: 0L
                            } ?: return@launch
    ...
    fun pause() {
        ...
        if (::player.isInitialized) {
            com.codetrio.overdrive.util.PlaybackStateManager.savePosition(applicationContext, player.currentPosition)
        }
        if (!wasPausedByFocusLoss) {
            abandonAudioFocus()
        }
                ...
                handler.post { 
                    ...
                    viewModel?.let { vm ->
                        if (vm.hapticManager == null) {
                            vm.hapticManager = com.codetrio.overdrive.util.PlayerHapticManager(applicationContext)
                        }
                    }
                }
            } catch (ex: Exception) {
    ...
    private fun setupMediaSession() {
        ...
        val intent = Intent(this, com.codetrio.overdrive.MainActivity::class.java)
    ...
    private fun setupPreferenceListener() {
        ...
        prefListener = SharedPreferences.OnSharedPreferenceChangeListener { _, key ->
            ...
            if (com.codetrio.overdrive.ui.SettingsViewModel.KEY_VOLUME_NORMALIZATION_ENABLED == key || 
                com.codetrio.overdrive.ui.SettingsViewModel.KEY_TARGET_LUFS == key) {
                updateBaseVolume()
            }
        }
    ...
    private fun updateBaseVolume() {
        val prefs = getSharedPreferences(com.codetrio.overdrive.ui.SettingsViewModel.PREFS_NAME,
            MODE_PRIVATE
        )
        val enabled = prefs.getBoolean(com.codetrio.overdrive.ui.SettingsViewModel.KEY_VOLUME_NORMALIZATION_ENABLED, false)
        ...
        val targetLufs = prefs.getFloat(com.codetrio.overdrive.ui.SettingsViewModel.KEY_TARGET_LUFS, -14f)
        ...
        if (song.lufs != null) {
            ...
            serviceScope.launch(Dispatchers.IO) {
                val measured = com.codetrio.overdrive.player.LoudnessAnalyzer.analyzeLufsFast(song.path)
                if (measured != null) {
                    // Update in-memory so we don't re-analyze
                    song.lufs = measured
                    // Save to playlist db if applicable
                    val db = com.codetrio.overdrive.data.db.MusicDatabase.getDatabase(applicationContext)
            ...
            serviceScope.launch(Dispatchers.IO) {
                val loudnessDb = com.codetrio.overdrive.data.innertube.YouTubeMusic.getLoudnessDb(song.videoId!!).getOrNull()
    ...
    private fun applyLufsGain(lufs: Float, targetLufs: Float) {
        val gain = com.codetrio.overdrive.player.VolumeNormalizer.calculateGain(lufs, targetLufs)
        val linearVolume = com.codetrio.overdrive.player.VolumeNormalizer.dbToLinear(gain)
